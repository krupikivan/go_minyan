...
class FirestoreProvider {
  ...
  Future<void> saveData(String documentId, String title, String address, String contact, double lat, double lon, List nusach) async {
  ...
  Future<void> saveNewTime(String documentID, String day, String pray, DateTime time, String dayDocumentId) async {
    await _firestore.collection(FS.markers).document(documentID).collection(FS.schedule).where(FS.name, isEqualTo: day).getDocuments()
    .then((doc) => _firestore.collection(FS.markers).document(documentID).collection(FS.schedule)
        .document(doc.documents[0].documentID).updateData(
...
//  }
  ...
  Future<void> deleteTime(String documentId, String day, String pray, Timestamp time, String dayDocumentId) async {
    await _firestore.collection(FS.markers).document(documentId).collection(FS.schedule).document(dayDocumentId).updateData({pray: FieldValue.arrayRemove([time])});
  }
...
//  }
  ...
  Stream<QuerySnapshot> getAllSchedule(String documentId) {
    return _firestore.collection(FS.markers)
        .document(documentId)
        .collection(FS.schedule).snapshots();
  }
  ...
  Future<QuerySnapshot> getMarkerDetails(String markID) {
    return _firestore.collection(FS.markers).document(markID).collection(FS.schedule).getDocuments();
  }

  ///Get data to verify push notification
  Stream<QuerySnapshot> getPushNotification(String documentId, String day) {
    return _firestore.collection(FS.markers).document(documentId).collection(FS.schedule).where(FS.name, isEqualTo: day)
