...
//    subs.cancel();
    ...
    showDialog(
      ...
      builder: (_) {
        ...
        return new AlertDialog(
          title: Text("$title"),
          content: Text("${this.newPush.place} - ${this.newPush.pray} : $payload"),
        );
      },
    );
  }
  ...
  _setPushData(PushData pushData){
    if(pushData != null){
      ...
      newPush.timeString = convertData.timestampToString(pushData.timeToPush.millisecondsSinceEpoch);
...
//             blocPush.changeFlag(true);
  ...
  void onTabTapped(int index) {
    ...
    if(index == 0){Navigator.pushNamed(context, '/minian');}
    if(index == 1){Navigator.pushNamed(context, '/minianForm');}
  ...
  _fillPopupData(){
    ...
    choices = <Items>[
      Items(Text(login, style: TextStyle(fontFamily: Theme.Fonts.primaryFont)), Icon(Icons.account_circle)),
      Items(Text(settings, style: TextStyle(fontFamily: Theme.Fonts.primaryFont)), Icon(Icons.settings)),
    ];
  }
}
