...
class UserDataBloc {
  ...
  Future submit(AppModel appModel) async{
    await _repository.saveData(_userData.value.documentId, _userData.value.title, _userData.value.address, _userData.value.contact, _userData.value.latitude, _userData.value.longitude, _userData.value.nusach);
...
//  }
  ...
  getUserDataFromFirebase(String userUID, AppModel appModel) {
    if( appModel.userData.title == null)
      ...
      _repository.userData(userUID).listen((snapshot){
        ...
        _repository.getAllSchedule(doc.documentID).listen((docTime){
          ...
          blocTimes.createSchedule(appModel);
        });
      });
    else {
      //If login was saved
